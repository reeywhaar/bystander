...
export function postFeedsExport(ids: string[]): ApiAction<Export> {
  return createApiAction((d) =>
    d.call({ method: "POST", path: "/api/feeds/export", body: { ids } }),
  );
}
...
export function postFeedsImport(
  feeds: ImportSelection[],
): ApiAction<ImportResult> {
  return createApiAction((d) =>
    d.call({ method: "POST", path: "/api/feeds/import", body: { feeds } }),
  );
}
